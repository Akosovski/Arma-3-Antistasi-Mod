...
if ((count _arevelar) > 2*({[_x,_marcador] call A3A_fnc_canConquer} count (units buenos))) exitWith {hint "The enemy still outnumber us, check the map and clear the rest of the area"};
//if (!isServer) exitWith {};
{
if ([_x] call A3A_fnc_canFight) then {[_x] spawn A3A_fnc_surrenderAction};
} forEach _arevelar;
{
if (isPlayer _x) then
	{
