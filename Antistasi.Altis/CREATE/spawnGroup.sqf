...
private ["_grupo","_cuenta","_cuentaRangos","_lider","_unidades","_index","_pos"];
...
for "_i" from 0 to (_cuenta - 1) do
	...
	if ((_i == 0) or (((_allUnits + 1) < maxUnits) and ((_allUnitsSide + 1) < _maxUnitsSide)) or _override) then
		{
		_pos = _posicion findEmptyPosition [2,50,SDKUnarmed];
		if (_pos isEqualTo []) then {_pos = _posicion};
		_unit = _grupo createUnit [(_tipos select _i), _pos, [], 0, "NONE"];
