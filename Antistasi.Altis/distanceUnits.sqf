...
if (_modo == 0) then
	{
	_result = allUnits select {(_x getVariable ["spawner",false]) and ((side group _x) in _variable) and (_x distance2D _referencia < _distancia)};
	}
else
	{
	{
	{
	_unit = _x;
	if ((_unit getVariable ["spawner",false]) and {_unit distance2d _referencia < _distancia}) exitWith {_result = true};
	} forEach (units _x);
	if (_result) exitWith {};
	} forEach _variable;
