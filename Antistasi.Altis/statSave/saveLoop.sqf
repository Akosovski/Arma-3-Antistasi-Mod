...
_hrfondo = (server getVariable "hr") + ({(alive _x) and (not isPlayer _x) and (_x getVariable ["spawner",false]) /*and ((group _x in (hcAllGroups theBoss) or (isPlayer (leader _x))) and (side group _x == buenos))*/} count (units buenos));
...
{
_amigo = _x;
if (_amigo getVariable ["spawner",false]) then
	{
