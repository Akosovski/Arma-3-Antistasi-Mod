...
private _friendlies = if (_lado == muyMalos) then {_lado} else {[_lado,civilian]};
_morteros = [];
_mgs = [];
			...
			if !(isNull _tanques) then
				...
				if (_allNearFriends findIf {_x call A3A_fnc_typeOfSoldier == "ATMan"} == -1) then
					...
					if (!(isNull _mortero) and ([_mortero] call A3A_fnc_canFight)) then
						...
						{
						if (!(isNull (_grupo getVariable ["artySupport",objNull])) and (speed _tanques < 1) and _hasRadio) then
							{
							if ({if (_x distance _tanques < 50) exitWith {1}} count _allNearFriends == 0) then {[(_grupo getVariable ["artySupport",objNull]),getPosASL _tanques,4] spawn A3A_fnc_artySupportAI};
							}
						else
							{
							if ((_lado != buenos) and _hasRadio and (_grupo getVariable ["lastQRFCall",0] < time)) then
			...
			if (_numObjetivos > 2*_numNearFriends) then
				...
				if !(isNull _cercano) then
					...
					if (!(isNull _mortero) and ([_mortero] call A3A_fnc_canFight)) then
						...
						{
						if (!(isNull (_grupo getVariable ["artySupport",objNull])) and _hasRadio) then
							{
							if ({if (_x distance _cercano < 50) exitWith {1}} count _allNearFriends == 0) then {[(_grupo getVariable ["artySupport",objNull]),getPosASL _cercano,1] spawn A3A_fnc_artySupportAI;};
							}
						else
							{
							if ((_lado != buenos) and _hasRadio and (_grupo getVariable ["lastQRFCall",0] < time)) then
