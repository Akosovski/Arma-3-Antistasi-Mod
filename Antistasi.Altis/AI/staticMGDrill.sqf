...
while {(alive _gunner)} do
	...
	if (!(_objetivos isEqualTo []) and (((_objetivos select 0) select 4) distance _gunner > 150))  then
		...
		if !(_esMortero) then
			{
			_index = _objetivos findIf {!((_x select 4) isKindOf "Tank") and (([objNull, "VIEW"] checkVisibility [eyePos (_x select 4), eyePos _gunner]) > 0)};
			if (_index != -1) then {_enemigo = (_objetivos select _index) select 4};
			/*
			{
			_eny = _x select 4;
		...
		if !(_mounted) then
			...
			if !(_gunner getVariable ["maniobrando",false]) then
				...
				if (([_gunner] call A3A_fnc_canFight) and ([_ayudante] call A3A_fnc_canFight)) then
					...
					if !(_esMortero) then {_gunner setVariable ["timeToBuild",time + 5]} else {_gunner setVariable ["timeToBuild",time + 30]};
					...
					if ([_gunner] call A3A_fnc_canFight) then
						...
						if (_esMortero) then
							{
							_grupo setVariable ["morteros",_gunner];
							{
							if ((!isNull (_x getVariable ["morteros",objNull])) and (!isNull (_x getVariable ["artySupport",objNull]))) then {_x setVariable ["artySupport",_veh]};
							} forEach (groups _lado)
							};
						sleep 60;
						};
					};
			...
			if (_gunner getVariable ["maniobrando",false]) then
				...
				if (([_gunner] call A3A_fnc_canFight) and ([_ayudante] call A3A_fnc_canFight)) then
					...
					if (alive _gunner) then
						...
						if (_esMortero) then
							{
							_grupo setVariable ["morteros",objNull];
							{
							if (!isNull (_x getVariable ["morteros",objNull]) and (_x getVariable ["artySupport",objNull] == _veh)) then {_x setVariable ["artySupport",objNull]};
							} forEach (groups _lado);
							};
						deleteVehicle _veh;
						};
