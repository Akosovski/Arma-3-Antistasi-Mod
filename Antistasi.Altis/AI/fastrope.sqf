private ["_veh","_grupo","_posicion","_posorigen","_heli","_landpos","_wp","_d","_wp2","_wp3","_xRef","_yRef","_reinf","_dist","_AAThreatEval"];
...
_AAThreatEval = if (count _this > 6) then {_this select 5} else {[0]};
...
*/
_landpos = [];
_dist = if (_reinf) then {30} else {300 + ((20*(_AAThreatEval select 0) min distanciaSPWN))};
...
waitUntil {sleep 1; (not alive _veh) or (_veh distance2D _landpos < 550) or !(canMove _veh)};
