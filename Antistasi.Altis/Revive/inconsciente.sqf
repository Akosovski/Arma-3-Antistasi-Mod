...
if (isPlayer _unit) then
	...
	if (_inPlayerGroup) then
		...
		if (_marcador != "") then
			{
			if (!([_marcador] call BIS_fnc_taskExists) and (lados getVariable [_marcador,sideUnknown] == buenos) and (_injurer in [malos,muyMalos])) then {[_marcador,_injurer,buenos] remoteExec ["A3A_fnc_underAttack",2]};
			};
		};
	};
