...
if ((dateToNumber date > _fechalimnum) or (isNull _camion)) then
	...
	while {(_cuenta > 0)/* or (_camion distance _posicion < 40)*/ and (dateToNumber date < _fechalimnum) and !(isNull _camion)} do
		{
		while {(_cuenta > 0) and (_camion distance _posicion < 40) and ({[_x] call A3A_fnc_canFight} count ([80,0,_camion,buenos] call A3A_fnc_distanceUnits) == count ([80,0,_camion,buenos] call A3A_fnc_distanceUnits)) and ({_x distance _camion < 50} count (units malos) == 0) and (dateToNumber date < _fechalimnum) and (isNull attachedTo _camion)} do
			{
		...
		if (_cuenta > 0) then
			{
			_cuenta = 120*_bonus;//120
			if (((_camion distance _posicion > 40) or (not([80,1,_camion,buenos] call A3A_fnc_distanceUnits)) or ({_x distance _camion < 50} count (units malos) != 0)) and (alive _camion)) then {{[petros,"hint","Don't get the truck far from the city center, and stay close to it, and clean all BLUFOR presence in the surroundings or count will restart"] remoteExec ["A3A_fnc_commsMP",_x]} forEach ([100,0,_camion,buenos] call A3A_fnc_distanceUnits)};
			waitUntil {sleep 1; ((_camion distance _posicion < 40) and ([80,1,_camion,buenos] call A3A_fnc_distanceUnits) and ({_x distance _camion < 50} count (units malos) == 0)) or (dateToNumber date > _fechalimnum) or (isNull _camion)};
			};
