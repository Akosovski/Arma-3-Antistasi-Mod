...
{
[_x] call A3A_fnc_FIAinitBASES;
[_x,"evac"] remoteExec ["A3A_fnc_flagaction",[buenos,civilian],_x];
_POWs pushBack _x
...
if ({alive _x} count _POWs == 0) then
	...
	} forEach (_POWs select {(alive _x) and (_x distance (getMarkerPos respawnBuenos) < 300)});
