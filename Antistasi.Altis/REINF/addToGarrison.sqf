...
private _max = [_cercano] call A3A_fnc_garrisonSize;
if (_max < (count (garrison getVariable _cercano)) + (count _unidades)) exitWith {hint format ["This garrison has no room for all those additional troops. Max troops is %1",_max]};
...
if (_unidades findIf {_x inArea _cercano} == -1) exitWith {hint "You need those units closer to the marker to add them as garrison troops"};
...
if (spawner getVariable _cercano != 2) then
	...
	_x addEventHandler ["killed",
		...
		[_muerto,"remove"] remoteExec ["A3A_fnc_flagaction",[buenos,civilian],_muerto];
