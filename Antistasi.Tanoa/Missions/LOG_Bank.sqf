...
waitUntil {sleep 1; (dateToNumber date > _fechalimnum) or (!alive _camion) or ((_camion distance _posicion < 7) and ({([_x] call A3A_fnc_canFight) and (_x inArea _mrk)} count (units malos) == 0))};
...
if ((dateToNumber date > _fechalimnum) or (!alive _camion)) then
	...
	if (_amigo distance _camion < 300) then
		...
		{_x reveal [_amigo,4]} forEach (units malos);
		};
	} forEach ([distanciaSPWN,0,_posicion,buenos] call A3A_fnc_distanceUnits);
	_exit = false;
	while {(_cuenta > 0) or ((_camion distance _posicion < 7) and ({([_x] call A3A_fnc_canFight) and (_x inArea _mrk)} count (units malos) == 0)) and (alive _camion) and (dateToNumber date < _fechalimnum)} do
		{
		while {(_cuenta > 0) and ((_camion distance _posicion < 7) and ({([_x] call A3A_fnc_canFight) and (_x inArea _mrk)} count (units malos) == 0)) and (alive _camion)} do
			{
