...
if (gameMode == 1) then
	...
	if (gameMode == 4) then
        ...
        if (gamemode == 3) then
            ...
            {
            _mrkCSAT = + aeropuertos + puertos;
            _mrkNato = marcadores - ["Synd_HQ"] - _mrkCSAT - puestos;
            {
            _puesto = _x;
            if (antenas findIf {_x inArea _puesto} != -1) then {_mrkNato pushBackUnique _puesto} else {_mrkCSAT pushBackUnique _puesto};
            } forEach puestos;
            {
            _nearest = [(aeropuertos + puestos),_x] call BIS_fnc_nearestPosition;
            if (_nearest in _mrkNATO) then {_controlesNATO pushBackUnique _x} else {_controlesCSAT pushBackUnique _x};
            } forEach controles;
            };
		};
	};
