...
private _AAThreatEval = if (count _this > 5) then {_this select 5} else {0};

_posicion = if (typeName _marcador == typeName "") then {getMarkerPos _marcador} else {_marcador};

_heli = group driver _veh;
{_x disableAI "TARGET"; _x disableAI "AUTOTARGET"} foreach units _heli;
_dist = 500 + ((100*_AAThreatEval) min distanciaSPWN);
...
while {true} do
   ...
   _randAng = _randAng + 1;
   _timeOut = _timeOut + 1;
   if (_timeOut == 360) exitWith {_landpos = _posicion};
	};
...
while {true} do
   ...
   _timeOut = _timeOut + 1;
   if (_timeOut == 360) exitWith {_exitPos = _landPos};
	};
...
_veh flyInHeight (300 + ((20 * _AAThreatEval) min 300));
