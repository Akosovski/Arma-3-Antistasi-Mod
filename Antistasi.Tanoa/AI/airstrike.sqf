...
if (_esMarcador) then
	...
	{
	_amigos = if (_lado == malos) then {allUnits select {(_x distance _posicion < 300) and (alive _x) and ((side _x == malos) or (side _x == civilian))}} else {(units muyMalos) select {(_x distance _posicion < 300) and (alive _x)}};
