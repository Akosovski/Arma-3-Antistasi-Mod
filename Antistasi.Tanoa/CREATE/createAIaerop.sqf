...
if (_patrol) then
	...
	while {(spawner getVariable _marcador != 2) and (_cuenta < 4)} do
		...
		if !(isNull _grupo) then
			{
			sleep 1;
			if ((random 10 < 2.5) and (not(_tipogrupo in sniperGroups)) and (tierWar < 8)) then
				{
...
private _maxVehs = if (!_busy) then {5} else {3};
...
if (count _buildings > 1) then
	...
	//_grupos pushBack _grupo;
	_cuenta = 0;
	while {(spawner getVariable _marcador != 2) and (_cuenta < _maxVehs)} do
		{
