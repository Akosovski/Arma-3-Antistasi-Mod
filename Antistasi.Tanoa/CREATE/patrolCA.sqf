...
if ((!_esMarcador) and (_typeOfAttack != "Air") and (!_super) and ({lados getVariable [_x,sideUnknown] == _lado} count aeropuertos > 0)) then
	...
	if ([_plane] call A3A_fnc_vehAvailable) then
		{
		_amigos = if (_lado == malos) then {allUnits select {(_x distance _posDestino < 200) and (alive _x) and ((side (group _x) == _lado) or (side (group _x) == civilian))}} else {(units _lado) select {(_x distance _posDestino < 100) and ([_x] call A3A_fnc_canFight)}};
...
if (!_inWaves) then
	...
	_aeropuertos = _aeropuertos select {([_x,_posDestino,_lado,[]] call A3A_fnc_AAThreatEval) select 0 < 5};
	...
	_puestos = if (_marcador in blackListDest) then {[]} else {if (_threatEvalLand <= 15) then {puestos select {(lados getVariable [_x,sideUnknown] == _lado) and ([_posDestino,getMarkerPos _x] call A3A_fnc_isTheSameIsland) and (getMarkerPos _x distance _posDestino < distanceForLandAttack)  and ([_x,true] call A3A_fnc_airportCanAttack)}} else {[]}};
...
if (_base != "") then
	...
	private _AAThreatEval = ([_aeropuerto,_marcador,_lado,[]] call A3A_fnc_AAThreatEval) select 0;;
	_vehPool = [];
	_cuenta = if (!_super) then {if (_esMarcador) then {2} else {1}} else {round ((tierWar + difficultyCoef) / 2) + 1};
	_tipoVeh = "";
		...
		if (not (_tipoVeh in vehTransportAir)) then
			...
			if (_esMarcador) then
				...
				if ((_marcador in aeropuertos)  or !(_veh isKindOf "Helicopter")) then
					{
					_proceder = false;
					[_veh,_grupo,_marcador,_aeropuerto,false,_AAThreatEval] spawn A3A_fnc_airdrop;
					}
				else
					{
					if (_esSDK) then
				...
				if !(_veh isKindOf "Helicopter") then
					{
					_proceder = false;
					[_veh,_grupo,_posDestino,_aeropuerto,false,_AAThreatEval] spawn A3A_fnc_airdrop;
					};
				};
			if (_proceder) then
				{
				_dist = 500 + ((_AAThreatEval * 100) min distanciaSPWN);
				_ang = ((_posDestino getDir _posOrigen) + (random 90) - 45);
				_landPos = [];
				while {_dist < distanciaSPWN} do
					{
					_provPos = _posDestino getPos [_dist,_ang];
					if (_enemigos findIf {_x distance _provPos < 500}==-1) exitWith {_landPos = _provPos};
					_dist = _dist + 50;
					};
...
{
_veh = _x;
if ((surfaceIsWater position _x) or (!([distanciaSPWN,1,_veh,buenos] call A3A_fnc_distanceUnits) and (({_x distance _veh <= distanciaSPWN} count (allPlayers - (entities "HeadlessClient_F"))) == 0))) then {deleteVehicle _x; _soldados = _soldados - [_x]};
} forEach _soldados;
