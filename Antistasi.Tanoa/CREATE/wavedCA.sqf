...
while {(_waves > 0)} do
	...
	_dir = 0;
	_spawnPoint = "";
	_AAThreatEval = ([_mrkOrigen,_mrkDestino,_lado,[]] call A3A_fnc_AAThreatEval) select 0; diag_log format ["Antistasi: WavedCA wave %1 with AAThreatEval of %2",_waves,_AAThreatEval];
	_attackOnlyLand = false;
	if !(_mrkDestino in blackListDest) then
		...
		if (_posOrigen distance _posDestino < distanceForLandAttack) then
			...
			if !(_puestos isEqualTo []) then
				...
				if (_AAThreatEval > 15) then
					{
					_nVeh = _nVeh*2;
					_attackOnlyLand = true;diag_log format ["Waved attack will be only performed from %1",_puesto];
					};
				};
			};
		};
	if !(_pos isEqualTo []) then
		...
		while {(_cuenta <= _nVeh) and (count _soldados <= 80)} do
			...
			if (_proceder) then
				{
				_timeOut = 0;
				_posTmp = _pos findEmptyPosition [0,100,_tipoveh];
				while {_timeOut < 60} do
					{
					if (count _pos > 0) exitWith {};
					_timeOut = _timeOut + 1;
					_posTmp = _pos findEmptyPosition [0,100+(10 * _timeout),_tipoveh];
					sleep 1;
					};
				if (count _posTmp == 0) then {_pos = getMarkerPos _spawnPoint} else {_pos = _posTmp};
	...
	if (([_mrkDestino,true] call A3A_fnc_fogCheck >= 0.3) and !_attackOnlyLand) then
		{
		...
		diag_log format ["Antistasi: WavedCA Air Spawn on wave %1. Vehpool: %2",_waves,_vehPool];
		_cuenta = 1;
		_pos = _posOrigen;
		_ang = 0;
		...
		while {(_cuenta <= _nVeh) and (count _soldados <= 80)} do
			...
			if ((_tipoVeh in vehTransportAir) and !(_spawnedSquad)) then
				...
				if (gameMode <3) then
					{
					_allUnitsSide = {(local _x) and (alive _x)} count (units _lado);
					_maxUnitsSide = round (maxUnits * 0.7);
					};
				if ((_allUnits + 4 > maxUnits) or (_allUnitsSide + 4 > _maxUnitsSide)) then
					{
					_proceder = false; diag_log format ["Antistasi: wavedCA on wave %1 wont spawn air veh because it reached the max level. Allunits: %2.Maxunits:%3.Allunitsside:%4.Maxunitsside:%5",_waves,_allUnits,maxUnits,_allUnitsSide,_maxUnitsSide];
					};
				};
			if (_proceder) then
				{
				...
				{[_x] call A3A_fnc_NATOinit} forEach units _grupoVeh;
				[_veh] call A3A_fnc_AIVEHinit;
				_veh flyInHeight 300 + ((_AAThreatEval * 20) min 800);
				if (not (_tipoVeh in vehTransportAir)) then
					...
					if (!(_veh isKindOf "Helicopter") or (_mrkDestino in aeropuertos)) then
						{
						[_veh,_grupo,_mrkDestino,_mrkOrigen,false,_AAThreatEval] spawn A3A_fnc_airdrop;
						}
					else
						{
						_ang = ((_posDestino getDir _posOrigen) + (random 90) - 45);
						_landPos = _posDestino getPos [500 + ((_AAThreatEval * 100) min distanciaSPWN), _ang];
						...
						if !(_landPos isEqualTo [0,0,0]) then
							...
							if ((_tipoVeh in vehFastRope) and ((count(garrison getVariable _mrkDestino)) < 10)) then
								{
								//_grupo setVariable ["mrkAttack",_mrkDestino];
								[_veh,_grupo,_posDestino,_posOrigen,_grupoVeh,false,_AAThreatEval] spawn A3A_fnc_fastrope;
								}
							else
								{
								[_veh,_grupo,_mrkDestino,_mrkOrigen,false,_AAThreatEval] spawn A3A_fnc_airdrop;
	...
	if (_lado == malos) then
		...
		if  ((({[_x,_mrkDestino] call A3A_fnc_canConquer} count _soldados) > 3*({(side _x != _lado) and (side _x != civilian) and ([_x,_mrkDestino] call A3A_fnc_canConquer)} count allUnits)) or (lados getVariable [_mrkDestino,sideUnknown] == malos)) then
			...
			if (_mrkDestino in ciudades) then
				...
				server setVariable [_mrkDestino,[_datos select 0, _datos select 1, 100, 0],true];
...
{
_veh = _x;
if ((surfaceIsWater position _x) or (!([distanciaSPWN,1,_veh,buenos] call A3A_fnc_distanceUnits) and (({_x distance _veh <= distanciaSPWN} count (allPlayers - (entities "HeadlessClient_F"))) == 0))) then {deleteVehicle _x; _pilotos = _pilotos - [_x]};
} forEach _pilotos;
{
_veh = _x;
...
{
_veh = _x;
if ((surfaceIsWater position _x) or (!([distanciaSPWN,1,_veh,buenos] call A3A_fnc_distanceUnits) and (({_x distance _veh <= distanciaSPWN} count (allPlayers - (entities "HeadlessClient_F"))) == 0))) then {deleteVehicle _x; _soldadosTotal = _soldadosTotal - [_x]};
