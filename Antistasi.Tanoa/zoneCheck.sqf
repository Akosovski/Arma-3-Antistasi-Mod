...
_winner = _enemy1;
if ({(_x getVariable ["spawner",false]) and ([_x,_marcador] call A3A_fnc_canConquer)} count (units _enemy1) <= {(_x getVariable ["spawner",false]) and ([_x,_marcador] call A3A_fnc_canConquer)} count (units _enemy2)) then {_winner = _enemy2};
