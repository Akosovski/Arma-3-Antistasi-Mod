...
private ["_objetivos","_marcadores","_base","_objetivo","_cuenta","_aeropuerto","_datos","_prestigeOPFOR","_scoreLand","_scoreAir","_analizado","_garrison","_size","_estaticas","_salir","_return","_sumulated","_origen","_destino"];

_simulated = if (count _this > 0) then {true} else {false};
_return = [];
_objetivos = [];
_marcadores = [];
_cuentaFacil = 0;
...
if !(_tmpTargets isEqualTo []) then
	...
	*/
	_nearestObjectives pushBack ([_tmpTargets,getMarkerPos _x] call BIS_fnc_nearestPosition);
	};
} forEach _aeropuertos;
//the following discards targets which are surrounded by friendly zones, excluding airbases, outposts and the nearest targets
//test: exclude cities in order to guarantee cities are a target no matter what
_objetivosProv = _objetivos - aeropuertos - _nearestObjectives - ciudades - puestos;
{
...
if (_objetivos isEqualTo []) exitWith {_return};
_objetivosFinal = [];
_basesFinal = [];
_cuentasFinal = [];
_objetivoFinal = [];
...
_tmpObjetivos = _objetivos select {lados getVariable [_x,sideUnknown] != _ladoBase};
...
//lets remove from the target list those outposts which are not the nearest one
_tmpOutposts = _tmpObjetivos select {_x in puestos};
if (count _tmpOutposts > 1) then
	{
	_tmpOutposts = _tmpOutposts - [[_tmpOutposts,_posBase] call BIS_fnc_nearestPosition];
	_tmpObjetivos = _tmpObjetivos - _tmpOutposts;
	};
...
if !(_tmpObjetivos isEqualTo []) then
	...
	if (_proceder) then
		...
		if (_times > 0) then
			...
			if (_isTheSameIsland) then
				...
				if !(_nearbyLandBases isEqualTo []) then
					...
					if  (!_esCiudad) then
						...
						{
						if (_ladoBase == malos) then {_times = _times *8};
						};
					};
				};
			_esMar = false;
			...
			//aquí va la resta por cuestiones de AA
			diag_log format ["El valor de times antes del AAEval para %1 es %2",_x,_times];
			if !(_base in ["NATO_carrier","CSAT_carrier"]) then
				{
				_eval = [_base,_x,_ladoBase,_alreadyEval] call A3A_fnc_AAThreatEval;
				_times = _times - (_eval select 0);
				_alreadyEval = _eval select 1;
				};
			_times = round (_times) max 0;
...
if (!_simulated) then {{[[_x select 0,_x select 1,"",false],"A3A_fnc_patrolCA"] remoteExec ["A3A_fnc_scheduler",2];sleep 30} forEach _faciles};
if (count _faciles > 2) exitWith {diag_log format ["Salimos porque hay los siguientes fáciles %1",_faciles]; _return};

if (hayIFA and (sunOrMoon < 1)) exitWith {_return};

_destino = "";
_origen = "";
_return = [];

if (!(_objetivosFinal isEqualTo []) and (count _faciles < 3)) then
	{
	_arrayFinal = [];
	...
	diag_log format ["Este es el array final %1 con estos pesos: %2",_arrayFinal,_cuentasFinal];
	_objetivoFinal = _arrayFinal selectRandomWeighted _cuentasFinal;diag_log format ["Objetivofinal: %1",_objetivoFinal];
	_destino = _objetivoFinal select 0;
	_origen = _objetivoFinal select 1;
	_return = [_origen,_destino];
	};
if (_simulated or (_return isEqualTo [])) exitWith {_return};
if (({!(side group _x in [buenos,civilian])} count allUnits) + 40 > maxUnits) exitWith {diag_log "No enviamos el ataque por haber muchas unidades en el mapa"};
...
if (_waves == 1) then
	...
	if (_destino in aeropuertos) then
		...
		{
		if (_destino in puestos) then {_waves = 2}
		};
	};
...
if ((_waves == 1) and (!_simulated)) then
	{
