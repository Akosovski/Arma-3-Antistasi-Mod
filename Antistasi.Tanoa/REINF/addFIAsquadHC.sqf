...
_tipoGrupo = if (_this isEqualType []) then {_this select 0} else {_this};
_exit = false;
...
if (_tipoGrupo isEqualType []) then
	...
	_costeHR = 2;
	_esInf = if ((activeGREF and ((_tipoGrupo == staticAABuenos) or (_tipoGrupo == vehSDKAT))) or (_tipoGrupo == vehSDKAT)) then {false} else {true};
...
if (_esinf) then
	...
	_format = format ["%1%2",_format,count (groups buenos)];
	...
	if (isNull (driver _camion)) then {(leader _grupo) assignAsDriver _camion};
	if (_tipogrupo == vehSDKAT) then {_grupo setGroupId [format ["M.AT-%1",count (groups buenos)]]};
	if (_tipogrupo == staticAABuenos) then {_grupo setGroupId [format ["M.AA-%1",count (groups buenos)]]};
