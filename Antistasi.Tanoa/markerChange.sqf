...
*/
if (lados getVariable [_marcador,sideUnknown] == _winner) exitWith {};
...
if (_winner == buenos) then
	...
	_statics = vehicles select {(_x inArea _marcador) and (_x isKindOf "StaticWeapon") and (isNull gunner _x) and !(_x getVariable ["staticsToSave",false])};
	{
	_x setVariable ["staticsToSave",true,true];
	} forEach _statics;
	}
else
	{
	_soldados = [];
	{_soldados pushBack (typeOf _x)} forEach ((units _winner) select {(_x distance _posicion < (_size*3)) and (_x getVariable ["spawner",false]) and (vehicle _x == _x) and (alive _x)});
	[_soldados,_winner,_marcador,0] remoteExec ["A3A_fnc_garrisonUpdate",2];
	_statics = vehicles select {(_x inArea _marcador) and (_x isKindOf "StaticWeapon") and (isNull gunner _x) and (_x getVariable ["staticsToSave",false])};
	{
	_x setVariable ["staticsToSave",false,true];
	} forEach _statics;
	};
...
if (_winner == buenos) then
	...
	waitUntil {sleep 1; ((spawner getVariable _marcador == 2)) or ({((side group _x) in [_looser,_other]) and (_x getVariable ["spawner",false]) and ([_x,_marcador] call A3A_fnc_canConquer)} count allUnits > 3*({[_x,_marcador] call A3A_fnc_canConquer} count (units buenos)))};
