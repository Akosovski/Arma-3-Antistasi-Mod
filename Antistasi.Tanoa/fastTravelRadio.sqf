		...
		if ((!isPlayer _unit) or (_unit == player)) then
			...
			if (_unit != vehicle _unit) then
				...
				if (driver vehicle _unit == _unit) then
					...
					while {_pos isEqualTo []} do
						...
						if (count _roads > 0) then
							{
							{
							_pos = position _x findEmptyPosition [0,_tam,typeOf (vehicle _unit)];
							if !(_pos isEqualTo []) exitWith {_dir = getDir _x};
							} forEach _roads;
							};
						_tam = _tam + 10;
						};
				...
				if ((vehicle _unit isKindOf "StaticWeapon") and (!isPlayer (leader _unit))) then
					{
					_pos = _posicion findEmptyPosition [0,100,typeOf (vehicle _unit)];
